...
TRUNCATE TABLE session_2_sql.hw_advanced_query;
...
ORDER BY category ASC, CASE status
...
SELECT category, CAST(AVG(price) as DECIMAL(10,1)) as average_price
FROM session_2_sql.hw_advanced_query
...
select category, (SUM(qty)) as total_items_sold
from session_2_sql.hw_advanced_query
group by category
having SUM(qty) > 3;
...
select customer_id, CAST(AVG(price) AS DECIMAL(10,1)) as avg_purchase_price
from session_2_sql.hw_advanced_query
group by customer_id
...
select customer_id, COUNT(*) as recent_purchases
from session_2_sql.hw_advanced_query
