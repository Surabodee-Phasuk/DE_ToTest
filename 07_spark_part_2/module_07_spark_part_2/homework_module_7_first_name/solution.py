...
step2_df = (
    step1_df.alias("o")
    .join(store_catalog.alias("c"), ["store_id"], "left")
...
step3_df = (
    ...
    .join(
        promotions.alias("p"),
        [
            (col("s.order_date") >= col("p.start_date"))
            ,(col("s.order_date") <= col("p.end_date"))
        ]
        ,"left"
    )
    .withColumn("discount_rate", coalesce(col("p.discount_rate"), lit(0.0)))
    .withColumn("final_amount", col("clean_amount") * (lit(1.0) - col("discount_rate")))
    .select("s.*", "p.promo_code", "final_amount")
)
display(step3_df)
...
window_user = Window.partitionBy("user_id").orderBy(col("order_date").desc())
window_user_agg = Window.partitionBy("user_id")

step4_df = (
    step3_df
    .withColumn("latest_order_rank", row_number().over(window_user))
    .withColumn("lifetime_value", sum("final_amount").over(window_user_agg))
)
display(step4_df)
