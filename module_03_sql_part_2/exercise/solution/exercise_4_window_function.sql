...
SELECT node_id, cpu_load, SUM(cpu_load) OVER (PARTITION BY node_id) AS total_node_load 
FROM session_3_sql.server_metrics
WHERE node_id = 1;
...
SELECT node_id, log_dt, ROW_NUMBER() OVER (PARTITION BY node_id ORDER BY log_dt ASC) AS rn 
FROM session_3_sql.server_metrics 
WHERE cpu_load > 40;
...
SELECT node_id, cpu_load, DENSE_RANK() OVER (PARTITION BY node_id ORDER BY cpu_load DESC) AS dense_rnk 
FROM session_3_sql.server_metrics 
WHERE node_id = 2;
...
SELECT node_id, log_dt, cpu_load, LAG(cpu_load) OVER (PARTITION BY node_id ORDER BY log_dt ASC) AS prior_load 
FROM session_3_sql.server_metrics 
WHERE log_dt BETWEEN '2026-04-01' AND '2026-04-02';
...
WITH base_metrics AS (
  SELECT * FROM session_3_sql.server_metrics
)
SELECT * FROM base_metrics WHERE cpu_load IN (45, 20);
...
SELECT node_id, log_dt, cpu_load, SUM(cpu_load) OVER (PARTITION BY node_id ORDER BY log_dt ASC) AS running_load 
FROM session_3_sql.server_metrics 
WHERE node_id = 1;
...
SELECT node_id, log_dt, cpu_load 
FROM session_3_sql.server_metrics 
WHERE log_dt >= '2026-04-01'
QUALIFY ROW_NUMBER() OVER (PARTITION BY node_id ORDER BY cpu_load DESC) = 1;
...
SELECT node_id, log_dt, (cpu_load - LAG(cpu_load) OVER (PARTITION BY node_id ORDER BY log_dt ASC)) AS load_spike 
FROM session_3_sql.server_metrics 
WHERE node_id = 2;
...
SELECT * FROM (
  SELECT *, RANK() OVER (ORDER BY cpu_load ASC) AS global_rank 
  FROM session_3_sql.server_metrics
) WHERE global_rank <= 2;