...
class SalesPipeline:
    ...
    def ingest_batches(self, *args):
        ...
        for batch in args:
            ...
            print(f"Ingested {len(batch)} records from batch {batch}")
    ...
    def extract_successful_sales(self):
        ...
        successful_sales = [record for record in self.raw_data if record.get("status") == "success"]

        # Step 3b: Dict Comprehension for transformation

        clean_sales = {record["txn_id"]: record["amount"] for record in successful_sales}
        return clean_sales
    ...
    def calculate_average_sale(self):
        ...
        try:
            average_sale = sum(clean_sales.values()) / len(clean_sales)
