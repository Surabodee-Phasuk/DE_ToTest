...
SELECT * FROM session_3_sql.campaign_jan
INTERSECT
SELECT * FROM session_3_sql.campaign_feb;
...
SELECT customer_email, 
       CASE WHEN segment = 'VIP' THEN 'Send Gift' ELSE 'Send Email' END AS action_plan 
FROM session_3_sql.campaign_jan 
WHERE customer_email LIKE '%mail.com';
...
SELECT customer_email, 
  CASE 
    WHEN customer_email LIKE 'b%' THEN 'High' 
    WHEN customer_email LIKE 'd%' THEN 'Low' 
    ELSE 'Unknown' 
  END AS priority 
FROM session_3_sql.campaign_feb 
...
SELECT customer_email FROM session_3_sql.campaign_jan WHERE segment = 'Standard'
EXCEPT
SELECT customer_email FROM session_3_sql.campaign_feb;
...
SELECT customer_email, 
  CASE 
    WHEN segment = 'Standard' THEN 
      CASE WHEN customer_email = 'c@mail.com' THEN 'Convert' ELSE 'Hold' END
    ELSE 'Reward' 
  END AS strategy
FROM session_3_sql.campaign_jan 
WHERE segment IN ('Standard', 'VIP');
...
SELECT customer_email, SUM(CASE WHEN segment = 'VIP' THEN 1 ELSE 0 END) AS vip_count
FROM session_3_sql.campaign_jan
WHERE customer_email LIKE '%a%'
