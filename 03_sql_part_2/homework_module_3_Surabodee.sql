...
TRY_CAST(raw_amount AS DECIMAL(10,2)) as amount,device
...
case
    ...
    when amount IS NULL then 'Failed'
    else 'Standard'
END as order_tier

from silver_orders s
inner join session_3_sql.dim_user_profiles u ON s.user_id = u.user_id
...
select 
    ...
    clean_dt,
    amount,
SUM(amount) OVER(PARTITION BY user_id ORDER BY clean_dt ASC) as running_revenue
from enriched_orders
where order_tier <> 'Failed';
...
user_id,
amount,
(amount - LAG(amount) OVER(PARTITION BY user_id ORDER BY clean_dt ASC)) as revenue_delta
...
enriched_orders as (
  ...
  from
    silver_orders s
      inner join session_3_sql.dim_user_profiles u
...
select
  ...
  from enriched_orders
qualify
  ROW_NUMBER() over (partition by user_id order by clean_dt DESC) = 1;
